...
def getImagesFormKonachan():
    url = "https://konachan.com/"

    numbers = [f"{url}/images/{x}.gif" for x in range(10)]
    ...
    for index, number in enumerate(numbers):
        ...
        if response.status_code == 200:
            ...
            for i, frame in enumerate(iter_frames(image)):
                frame.save(f"{index}.png",**frame.info)
            image = Image.open(f"{index}.png").convert('RGBA')
            svg_image = rgba_image_to_svg_contiguous(image)
            with open(f"{index}.svg", "w") as text_file:
